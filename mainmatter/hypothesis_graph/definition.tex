...
(completed_test_num) edge node[left]{create system model} (completed);
    \draw [-stealth] (completed_test_num) edge[bend right] node[right]{goto next start configuration} (init_test_num);
    \draw [-stealth] (completed_test_num) to node[]{Unable to reach next start configuration}  (failed.west);
