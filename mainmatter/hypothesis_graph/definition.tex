...
A \textbf{identificationEdge}, \todo[inline]{define identificationEdge}
...
An identificationEdge \todo[inline]{identification edge intro}, the status can be visualised in \cref{tikz:status_identification_edge}.\bs
...
\begin{tikzpicture}[node distance = 2cm, auto, initial]
 ...
 % arrows
    \draw [-stealth] ([xshift=-2cm]init_test_num.west) to node[near start,above]{create test inputs} (init_test_num.west);
...
The second type of edge is an actionEdge. An actionEdge ready for execution contains all necessary information to sent input to the robot resulting in an object being steered toward it's target state. Before an edge is ready for execution it should be initialised properly, more specifically: initialised, path estimated should be performed, a system model must be initilised and path planned must be performed. Then finally the edge is ready to be executed and send input toward the robot, an \ac{FSM} of the actionEdge's status can be visualised in \cref{tikz:status_action_edge}.\bs
...
\end{figure}
\todo[inline]{what is going on with your colors? choose a color palette for the entire thesis document please. }
...
\todo[inline]{the FSM above, it is not clear if that is about pushing, driving, what is an action.}
\todo[inline]{add reason why to split action and identificatino edges}
