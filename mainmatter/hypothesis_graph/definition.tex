...
An object holds the information about an object.\\Formally, a \textbf{object},  $\mathit{obj}_{id}(k) = \left\langle s(k), \mathit{shape} \right\rangle $\bs

where $\mathit{shape}$ is linked to a 3D representation of the object.\bs

An object node represents an object in a state.\\Formally, a \textbf{objectNode}, $V^{\mathit{obj}}_{id} =\left\langle \textrm{status}, \mathit{obj}(k)\right\rangle $.\bs
...
A \textbf{action edge}, \[\tau_{(from, to)} = \left\langle \textrm{status}, id_{from}, id_{to}, \textrm{verb}, \textrm{controller},\textrm{dynamic model}, \textrm{path}\right\rangle\]\bs
...
\\comprising $V = \{V^{\mathit{obj}}_{i}\}$, \quad $E \in \{\tau_{(i,j)}| V_i, V_j \in \{V^{\mathit{obj}} \}, i \neq j\}$.\bs
...
\begin{tikzpicture}[node distance = 2cm, auto, initial]
 ...
 % arrows
    ...
    \draw[-stealth] (init_test_num) edge[bend right] node[left]{\shortstack{Go to next start\\push configuration}} (completed_test_num)
(completed_test_num) edge node[left]{Create system model} (completed);
    \draw [-stealth] (completed_test_num) edge[bend right] node[right]{Collect \ac{IO} data} (init_test_num);
...
\begin{enumerate}
  \item[INITIALISED (IN)] The edge is created with a source and target node which are present in the \ac{hgraph}. A choice of controller is made.
    \item[PATH EXISTS (PE)] A graph-based search is performed to validate if the target configuration is reachable assuming that the system is holonomic.
    \item[SYSTEM MODEL (SM)] A dynamics system model is provided to the controller residing in the edge.
    \item[PATH PLANNED (PP)] Resulting from a sample-based planner, a path from start to target configuration is provided.
    \item[EXECUTING (EX)] The edge is currently receiving observations from the robot environment and sends back robot input.
    \item[COMPLETED (COMPL)] The edge has driven the system toward its target configuration and its performance has been calculated.
...
Before executing edges, edges must be initialised, which is where next section is dedicated to.\bs
