...
Formally, a \textbf{hypothesis graph}, $\gls{hgraph} = \left\langle \gls{nodesG}, \gls{edgesG} \right\rangle $
\\comprising $\gls{nodesG} = \{\gls{nodesG}^{\mathit{obj}}_{i}\}$, \quad $\gls{edgesG} \in \{\gls{edge}_{(i,j)}| \gls{edgesG}_i, \gls{edgesG}_j \in \{\gls{nodesG}^{\mathit{obj}} \}, i \neq j\}$.\bs
...
\todo[inline]{right align this a bit nicer please}
\begin{flushright}
\centerline{\begin{minipage}{.6\textwidth}
\begin{enumerate}
    \item[PRE INPUT SEQUENCE number t (PrIS\#t)] Go to target configuration to input the input sequence on the system.
    \item[POST INPUT SEQUENCE number t (PoIS\#t)] Collect the output sequence.
    \item[COMPLETED (CO)] The edge has driven the system toward its target configuration and its performance has been calculated.
