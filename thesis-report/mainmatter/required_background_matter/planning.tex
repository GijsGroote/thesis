\section{Planning}%
\label{sec:planning}
\todo{Gijs: Local planning (connecting 2 configurations, or last minute real time planning), path planning -> path estimation and motion/manipulation palnning, motion planning/manipulation planning -> action planning, global planning -> finding an actino sequence. How can you make this clear to the reader??}

\todo{Corrado: Do not say planner, make sure the call it a action, global or local planner!}
This section explains planning for a single drive or push action as \textit{path planning}. Path planning consists of 2 steps, firstly, path estimation, and secondly, motion or manipulation planning. The path estimator can detect non-existent paths and concludes such paths as unfeasible. \todo{Corrado: what is the remainder of actions?} For the remainder of actions, motion or manipulation planning is responsible for finding a path from starting point to a target point in configuration space. The planner has to account for multiple subspaces in configuration space (free, obstacle, movable and unknown space). An existing motion planner~\cite{chen_fast_2018} extends to incorporate movable and unknown spaces next to the conventional free and obstacle spaces. \todo{Corrado: If this is new, put it in new. sepearte from required background an proposed shit} The modification incentivizes the planner to find a path in free space but can pass through unknown or movable space as a last resort. The robot should first remove the blocking objects if a planned path crosses an unknown or movable subspace.\bs
...
\todo{Corrado: Maybe make clear that your space is 2d}
...
\todo{Corrado: The configuration space for a 3d object is not 2d. You should say that you assume to work on a plane and thus you can represent the configuration space of a sphere or cuboid as it’s 2d projection on the plane, assuming cuboids do not tip over }
...
\todo{Gijs: maybe make this the same as the cell thingy here below, with x and y}
...
\todo{Corrado: What is an higher order of importantece, just say what you want to say, no fancy fancy pleas}
...
\todo{Corrado: put this earlier, it avoids confusions}
For rectangular objects, the orientation of the object becomes important because the orientation, together with the $\mathit{(x, y)}$ coordinates, determines which subspace the object resides. The configuration space for rectangular objects is thus 3-dimensional, whilst circular objects have a 2-dimensional configuration space.\bs
...
\todo{Corrado: Will I find somewhere the details about this? They are required for me to be able to reproduce this, GIJS: related to the shortest path, cite dijkstra please.}
...
\todo{Corrado: can be converted? yeah,cool how? How, and what does this mean, create an example please}
...
\todo{Corrado: What’s the difference between path estimator and path existence algorithm?, GIJS: CHECK all please now}
...
The path estimator suffers from another drawback, finding proof that there exists a path that is undecidable~\cite{zhang_simple_2008}. This is due to the chosen cell size during discretizing the configuration space. An example is a corridor having the same width as the robot. The robot fits exactly through this corridor. Detecting such a path requires many neighbouring cells that lie exactly in the centre line of the corridor. Only with a cell size going to zero and the number of cells going to infinity such a path is guaranteed to be detected. Path non-existence, on the other hand, is easy to prove because the path estimation algorithm provides an upper bound on existing paths and a lower bound on non-existing paths~\cite{zhang_simple_2008}.\bs
\todo{Corrado: I don’t understand this. How are the bounds connected to the fact that you can prove existence? jj}

\todo{Corrado: I’m not following, does the path existence algorithm consider non holonomic constraints? If so, how is this different than a global planner? GIJS: about the non-existend paths}
The path existence algorithm can detect non-existence paths. Thus checking path existence before motion or manipulation planning filters out several non-existent paths saving time and resources. \todo{Corrado: Even if,...The sentence does not fit the flow } Even if, in exceptional cases, the path estimation algorithm can yield unfeasible paths and can fail to detect existing paths. Checking path existence before path planning filters some non-exist paths and is additionally motivated by two reasons. First, path estimation is orders of magnitude faster compared to motion or manipulation planning. Secondly, the path estimation algorithm can provide a number of initial samples to the motion or manipulation planner that can act as a \quotes{warm start}.
...
\todo{Corrado: Move the italic stuff to before the additional modification. And make it entirely go to the proposed method. That makes sure that is is aaaa god damn addition, jeez/Tu}
...
\todo{Corrado: You need to use visuals as you go and not dump everything at the end.}
