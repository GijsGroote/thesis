...
\todo{Corrado: Also this subsection needs restructuring. It is very much unreadable if all definitions are given at once, I can’t memorize them

I would do the same as in one of my previous comments, that is, explain step by step what’s going on first, introduce in the text the terminology, then show the algorithm pseudo code and then summarize the terms in the table.

Example: the algorithm is composed by two main parts, a search loop and an execution loop. In the search loop …}
...
\todo{Corrado: This is a lot to ask to a reader, you have the tendency so summarize everything in big tables and pictures and that’s okay if you are 100 on top of the topic but most readers will have no clue about what you did, so you need to help them build incrementally an idea of your method}
