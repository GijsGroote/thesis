...
\todo{Martijn: This non-uniqueness doesn't sound very special to me, all redundant robots have that. All manipulators that need to reach a 3D point in space without caring for the orientation, have that.}

% What's Purpose: multiple modes of dynamics, piecewise analytic
Finding an optimal solution to a \ac{NAMO} and pushing task requires a search in the joint configuration space. Apart from the fact that the joint configuration space becomes exponentially larger with every increase of objects, there is another problem.
...
\todo{Martijn: YOU are repeating yourself}
...
\todo{GIJS: remove these in text quesitons, that is confusing Martijn: these in text questioins arethese are not subquestions of the main question}
The main research question is split into two smaller more detailed subquestions. Essentially the first research subquestion asks \quotes{how does the proposed framework work?}, which allows to explain the proposed framework. The second research subquestion asks: \quotes{How does it compare to state-of-the-art?}, allowing to compare the proposed frameworks with existing state-of-the-art.\bs


\textbf{Research subquestion:}
\begin{enumerate}
    \item\label{researchsubquestion:does_it_work} Can the proposed framework combine learning and planning for push en drive applications with a technique known as backward search?
...
\todo{remove questions is the text that are close to the research subquestions}
...
\todo{you select 3-dimensionsl environment twice?}
To answer the research questions, tests will be performed in a robot environment. A simple environment is desired because that simplifies testing, yet the robot environment should represent many real-world environments in which robots operate, thus a 3-dimensional environment is selected. The environment consists of a flat ground plane since many mobile robots operate in a workspace with a flat floor, such as a supermarket, warehouse or distribution center. The robots to test should be flat robots that have a low center of gravity, which lowers the chance of tipping over. A 3-dimensional environment is selected.\todo{Gijs Expand on mostly which environments are exceptions on this. And which are included?} Mostly the environment with a flat floor and a flat robot can be treated as a 2-dimensional problem because the robot and objects can only change position over $x$ and $y$ axis ($xy$ plane parallel to the ground plane) and rotate around the $z$ axis (perpendicular to the ground plane).\bs
...
\todo{Make sure walls are in set of objects, but also taht that is communicatedd}
\noindent
\begin{table}[H]
\centering
\begin{tabular}
...
\gls{motionEquations}& A set of motion equations describing the behavior of objects. In recent literature, the motion equations are also named the true dynamics. \vspace{0.5\baselineskip}\\
\end{tabular}
\end{table}
