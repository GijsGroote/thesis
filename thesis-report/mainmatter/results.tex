...
\todo{Corrado: above sentece Feels contradictory }
\todo{Corrado: For every plot put the control parameterisation on every plot!}
% The previous 3 benchmark tests are designed around the proposed framework, meaning that these tests highlight the strong points of the \ac{halgorithm} but prevent the testing of the weak points of the proposed frameworks. Such bias can lead to false positives, which are undesired.
...
\todo{add to the table below: max speed = 1.0, known obstacle cost = 2, unknown obst cost = 3.5}
\begin{center}
\begin{align*}
...
\end{figure}
\todo{Martijn: To do that properly, you should have done so many experiments that this influence of random path lengths is averaged out. Gijs: reach statistical relevance please}

\todo{Corrado: Show this in a graph. There is no mention to MPC or mppi in the graph make such a graph!}
