...
\end{tabular}
\caption{Method metrics employed to measure task performance by the proposed robot framework in the left column. Motivation on relevance of th metric is provided in the corresponding right column.}\label{table:proposed_method_metrics}
\end{table}
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Average execute-, search and total times for a task that involves learning object dynamcis and the \ac{NAMO} problem. The results are average over three solved tasks for \citeauthor{wang_affordancebased_2020}, and ten solved tasks for the proposed framework. A visualization of the task if presented in \Cref{fig:wang}.}%
